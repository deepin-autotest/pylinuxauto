...
from pylinuxauto.attr.dogtail.logging import TimeStamp
...
def run(string, timeout=config.runTimeout, interval=config.runInterval, desktop=None, dumb=False, appName=''):
    ...
    """
    if not desktop:
        from pylinuxauto.attr.dogtail.tree import root as desktop
                    ...
                    if child.name == appName:
                        for grandchild in child.children:
                            if grandchild.roleName == 'frame':
                                from pylinuxauto.attr.dogtail.procedural import focus
